...
uniform float _TessMinPixels;
uniform float _TessMaxPixels;
...
// ------------------------------------------------------
// Screen-space adaptive tesselation 
// ------------------------------------------------------


// helper: project a world-space position to clip space (view-projection)
float4 ProjectToHClip(float3 worldPos)
{
    // UNITY_MATRIX_VP is view * projection
    // TransformWorldToHClip exists in your included URP helpers; either use it or:
    return mul(UNITY_MATRIX_VP, float4(worldPos, 1.0));
}

float ScreenEdgeTessFactor(float3 w0, float3 w1, float minPixels, float maxPixels, float maxTess)
{
    float4 c0 = ProjectToHClip(w0);
    float4 c1 = ProjectToHClip(w1);

    // NDC
    float2 ndc0 = c0.xy / c0.w;
    float2 ndc1 = c1.xy / c1.w;

    // convert to pixel coords using _ScreenParams (x = width, y = height)
    float2 p0 = (ndc0 * 0.5 + 0.5) * _ScreenParams.xy;
    float2 p1 = (ndc1 * 0.5 + 0.5) * _ScreenParams.xy;

    float pixelLen = length(p0 - p1);

    // map pixel length to tess factor
    float t = saturate((pixelLen - minPixels) / (maxPixels - minPixels));
    return lerp(1.0, maxTess, t); // 1..maxTess
}

// blended patch constant using screen-space edges
TessellationFactors patchConstantFunction(InputPatch<ControlPoint, 3> patch)
{
    // world-space control points
    float3 w0 = mul(unity_ObjectToWorld, patch[0].vertex).xyz;
    float3 w1 = mul(unity_ObjectToWorld, patch[1].vertex).xyz;
    float3 w2 = mul(unity_ObjectToWorld, patch[2].vertex).xyz;

    // screen-space thresholds (pixels) - tune these in inspector or constants
    float minPixels = _TessMinPixels;
    float maxPixels = _TessMaxPixels;

    float e0 = ScreenEdgeTessFactor(w0, w1, minPixels, maxPixels, _Tess);
    float e1 = ScreenEdgeTessFactor(w0, w2, minPixels, maxPixels, _Tess);
    float e2 = ScreenEdgeTessFactor(w1, w2, minPixels, maxPixels, _Tess);

    TessellationFactors f;
    // combine into edges/inside similar to UnityCalcTriEdgeTessFactors usage
    float3 vs = float3(e0, e1, e2);
    f = UnityCalcTriEdgeTessFactors(vs);
    return f;
}
